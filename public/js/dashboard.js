...
function renderTable(pas) {
  ...
  pas.forEach((pa) => {
    ...
    if (pa.pa_status.toLowerCase() === "denied") {
      tr.classList.add("bg-red-100", "text-red-800");
    } else if (pa.pa_status.toLowerCase() === "approved") {
      tr.classList.add("bg-green-100", "text-green-800");
    }

    tr.innerHTML = `
      <td class="px-4 py-2">
	  	<a href="https://dashboard.covermymeds.com/v2/requests/${
        pa.pa_id
      }" target="_blank">
        	${pa.pa_id}
		</a>
      ...
      <td class="px-4 py-2">${formatDate(pa.patient_dob)}</td>
      <td class="px-4 py-2">${pa.drug_name}</td>
      <td class="px-4 py-2">${formatDate(pa.submitted_at)}</td>
      <td class="px-4 py-2">
	  	<a href="https://khasak.ema.md/ema/web/practice/staff#/practice/staff/patient/${
        pa.ema_id
      }/chart/overview" target="_blank">
	  		${pa.ema_id}
		</a>
	  </td>
	  <td class="px-4 py-2">${pa.insurance}</td>
      <td class="px-4 py-2">
        <select class="status-select border px-2 py-1 rounded" data-id="${
          pa.pa_id
        }">
